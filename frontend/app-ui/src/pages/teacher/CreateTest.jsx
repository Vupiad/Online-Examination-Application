...
const CreateTest = () => {
  ...
  // Helper counts for the selector
  ...
  const handleAddQuestion = (type = "MULTIPLE_CHOICE") => {
    const newQuestion = {
      ...
      testCases: type === "CODE" ? [
        { id: Date.now() + 1, input: "", expectedOutput: "", points: 5, isHidden: false }
      ] : [],
      starterCode: "",
      language: "Python",
    };
  ...
  const handleDuplicateQuestion = (index) => {
    const qToCopy = { ...questions[index], id: Date.now() };
    const newQs = [...questions];
    newQs.splice(index + 1, 0, qToCopy);
    setQuestions(newQs);
  };
  ...
  return (
    <div className="flex-1 min-w-0 flex flex-col relative bg-[#f7fafc] font-['Inter']">
        ...
        <section className="max-w-4xl mx-auto space-y-8 pb-32">
          {questions.map((q, qIndex) => (
            <div key={q.id} className="flex flex-col">
              {/* Type Selector for EACH question */}
              <QuestionTypeSelector 
                currentType={q.type} 
                onTypeChange={(newType) => handleUpdateQuestion(qIndex, { ...q, type: newType })}
                counts={qIndex === 0 ? questionCounts : {}} // Show counts only on first or globally? Image shows it globally.
              />

              {q.type === "MULTIPLE_CHOICE" && (
                <div className="bg-white rounded-[2.5rem] shadow-sm relative overflow-hidden group mb-8 border border-[#aab3b8]/10">
                  <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${qIndex === 0 ? "bg-[#026880]" : "bg-[#aab3b8]/30"}`}></div>
                  <div className="p-8 md:p-12">
                    <div className="flex justify-between items-start mb-6">
                      <div className="flex items-center gap-6">
                        <span className="text-3xl font-light text-[#005b70]/40 font-['Be_Vietnam_Pro'] tracking-tighter">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => handleDuplicateQuestion(qIndex)} className="p-2 text-[#737c80] hover:text-[#026880] transition-colors">
                          <Copy size={20} />
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="space-y-6">
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-[#737c80] uppercase tracking-wider ml-1">Question Content</label>
                        <textarea
                          value={q.content}
                          ...
                          <div key={opt.id} className={`flex items-center gap-3 p-4 rounded-xl transition-all border ${opt.isCorrect ? "bg-[#94dffb]/20 border-[#026880]" : "bg-[#f7fafc] border-transparent hover:bg-[#e2e9ed]"}`}>
                            ...
                            <div className="flex-1">
                              <input
                                ...
                                className="w-full bg-transparent border-none p-0 focus:ring-0 text-[#2b3437] outline-none font-medium"
          ...
          {/* Nút thêm câu hỏi - Bây giờ có thể chọn loại */}
          <div className="flex justify-center gap-4 mt-12 py-8 border-t-2 border-dashed border-[#dbe4e9]">
             <button onClick={() => handleAddQuestion("MULTIPLE_CHOICE")} className="px-6 py-3 bg-white text-[#026880] rounded-full font-bold shadow-sm border border-[#aab3b8]/20 flex items-center gap-2 hover:bg-[#f7fafc] transition-all">
                <PlusCircle size={18} /> Add Multiple Choice
             </button>
             <button onClick={() => handleAddQuestion("ESSAY")} className="px-6 py-3 bg-white text-[#026880] rounded-full font-bold shadow-sm border border-[#aab3b8]/20 flex items-center gap-2 hover:bg-[#f7fafc] transition-all">
                <PlusCircle size={18} /> Add Essay
             </button>
             <button onClick={() => handleAddQuestion("CODE")} className="px-6 py-3 bg-[#026880] text-white rounded-full font-bold shadow-lg shadow-[#026880]/20 flex items-center gap-2 hover:opacity-90 transition-all">
                <PlusCircle size={18} /> Add Code Question
             </button>
          </div>
        </section>
      </div>
      ...
      <footer className="sticky bottom-0 left-0 right-0 p-6 bg-white/90 backdrop-blur-2xl border-t border-[#e2e9ed] flex justify-center items-center z-40">
        <div className="max-w-4xl w-full flex justify-between items-center gap-4">
          <div className="flex items-center gap-4">
            <div className="px-5 py-2.5 bg-[#cee7ec] rounded-full text-xs font-bold text-[#026880] uppercase tracking-widest">
              {questions.length} Items
            </div>
          </div>
          <div className="flex gap-4">
            <button className="px-8 py-3 bg-[#e2e9ed] text-[#4c6367] rounded-full font-semibold hover:bg-[#dbe4e9] transition-all font-['Be_Vietnam_Pro'] px-8">
              Preview
            </button>
            <button
              onClick={handleSaveTest}
              disabled={isSaving}
              className="px-12 py-3 bg-[#026880] text-white rounded-full font-bold shadow-xl shadow-[#026880]/30 flex items-center gap-2 hover:scale-105 transition-all disabled:opacity-70 font-['Be_Vietnam_Pro']"
            >
              <Save size={20} />
