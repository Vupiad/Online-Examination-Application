...
const TeacherDashboard = () => {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const statsRes = await api.get('/api/exams/stats');
    ...
    <div className="flex bg-[#f8fafc] min-h-screen">
      ...
      <aside className="sidebar">
        <div className="p-6 mb-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-grey m-0">Hệ thống quản lý thi</p>
            </div>
          </div>
          <div className="mt-8">
            <h3 className="text-md font-bold m-0">Chào thầy cô</h3>
            <p className="text-xs text-grey">Hệ thống quản lý thi</p>
          </div>
        </div>

        <nav className="flex-grow">
          <Link to="/teacher/dashboard" className="sidebar-item active">
        ...
        <div className="p-4 mt-auto">
          <button 
            className="btn btn-primary mb-4 py-3"
            onClick={() => navigate('/teacher/create-exam')}
          >
            <Plus size={20} /> Tạo đề thi mới
          </button>
          <button 
            className="sidebar-item w-full bg-transparent border-none cursor-pointer"
            onClick={() => {
